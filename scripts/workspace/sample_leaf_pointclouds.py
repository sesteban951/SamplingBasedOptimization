...
def _parse_args() -> SampleConfig:
    ...
    parser.add_argument("--out", type=Path, default=Path("results/pointclouds/leaf_pointclouds.npz"), help="Output npz path.")
    parser.add_argument(
        "--max-attempts",
        type=int,
        default=100000,
...
def main() -> None:
    ...
    _save_pointclouds(cfg.out_path, leaf_names, points)
    print(f"Saved {len(leaf_names)} leaf point clouds with {cfg.num_samples} samples each to {cfg.out_path}.")
