...
def linear_annealing(itr, I, alpha_m=1.0):
    ...
    Args:
        ...
        alpha_m: (float) Maximum value of the annealing coefficient.
    Returns:
...
def cosine_annealing(itr, I, alpha_m=1.0):
    """
    Cosine annealing schedule.
    α(i) = αₘ * 0.5 * (1 + cos(π * i / I))
    where: αₘ ∈ [0, 1] is the maximum value of the annealing coefficient
    ...
    # compute the annealing coefficient
    alpha = 0.5 * alpha_m * (1 + np.cos(np.pi * itr / I))
...
def tanh_annealing(itr, I, alpha_m=1.0, sigma=5.0):
    """
    ...
    alpha = 0.5 * alpha_m * (1 - term1 / term2)
...
if __name__ == "__main__":
    ...
    for i in range(I_tot):
        alpha_linear[i] = linear_annealing(i, I_tot, alpha_m=alpha_max)

    # ------------------- Exp Annealing -------------------
    alpha_exp = np.zeros(I_tot)
    for i in range(I_tot):
        alpha_exp[i] = exponential_annealing(i, I_tot, alpha_m=alpha_max, lam=0.05)

    # ------------------- Cosine Annealing -------------------
    alpha_cosine = np.zeros(I_tot)
    for i in range(I_tot):
        alpha_cosine[i] = cosine_annealing(i, I_tot, alpha_m=alpha_max)

    # ------------------- Tanh Annealing -------------------
    alpha_tanh = np.zeros(I_tot)
    for i in range(I_tot):
        alpha_tanh[i] = tanh_annealing(i, I_tot, alpha_m=alpha_max, sigma=5.0)
